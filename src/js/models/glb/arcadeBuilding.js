...
export function criarArcadeBuilding(scene) {
  ...
  // ── DECORAÇÃO EXTRA / PREENCHIMENTO (NOVO) ──────────────────────────────────

  // 1. Dutos de ventilação no teto
  const dutoPrincipal = new THREE.Mesh(
    new THREE.CylinderGeometry(2, 2, LARGURA - 10, 8),
    matMetalDuto,
  );
  dutoPrincipal.rotation.z = Math.PI / 2;
  dutoPrincipal.position.set(0, ALTURA - 5, -10); // Perto do teto, recuado
  buildingGroup.add(dutoPrincipal);

  const dutoTransversal = new THREE.Mesh(
    new THREE.CylinderGeometry(1.5, 1.5, PROFUNDIDADE - 10, 8),
    matMetalDuto,
  );
  dutoTransversal.rotation.x = Math.PI / 2;
  dutoTransversal.position.set(LARGURA / 4, ALTURA - 5, 0);
  buildingGroup.add(dutoTransversal);

  // 2. Decoração de Parede Traseira (Painéis geométricos retro)
  const decoTraseiraGroup = new THREE.Group();
  decoTraseiraGroup.position.set(0, ALTURA * 0.6, -(PROFUNDIDADE / 2));

  // Painel central letreiro vazio
  const painelLetreiro = new THREE.Mesh(
    new THREE.BoxGeometry(30, 10, 0.5),
    matLetreiro,
  );
  painelLetreiro.position.z = 0.5;
  decoTraseiraGroup.add(painelLetreiro);

  // Moldura letreiro
  const molduraLetreiro = new THREE.Mesh(
    new THREE.BoxGeometry(32, 12, ESPESSURA * 0.6),
    matMetal,
  );
  molduraLetreiro.position.z = 0.1;
  decoTraseiraGroup.add(molduraLetreiro);

  // Formas geométricas laterais (estilo fantasmas Pacman simplificados)
  const fantasmaGeom = new THREE.BoxGeometry(6, 6, 0.5);
  const decoEsq = new THREE.Mesh(fantasmaGeom, matDeco1);
  decoEsq.position.set(-25, 0, 0.5);
  decoTraseiraGroup.add(decoEsq);

  const decoDir = new THREE.Mesh(fantasmaGeom, matDeco2);
  decoDir.position.set(25, 0, 0.5);
  decoTraseiraGroup.add(decoDir);

  buildingGroup.add(decoTraseiraGroup);

  // 3. Caixas de energia/fusíveis na parede esquerda
  const caixaEnergiaGroup = new THREE.Group();
  caixaEnergiaGroup.position.set(-(LARGURA / 2), balcaoAltura * 1.5, 0);

  const caixaGeom = new THREE.BoxGeometry(ESPESSURA * 0.8, 8, 6);
  const caixa1 = new THREE.Mesh(caixaGeom, matMetalDuto);
  caixa1.position.set(ESPESSURA * 0.4, 0, -10);
  caixaEnergiaGroup.add(caixa1);

  const caixa2 = new THREE.Mesh(caixaGeom, matMetalDuto);
  caixa2.position.set(ESPESSURA * 0.4, -2, -2);
  caixaEnergiaGroup.add(caixa2);

  buildingGroup.add(caixaEnergiaGroup);
